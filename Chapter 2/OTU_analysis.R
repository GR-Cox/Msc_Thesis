...
Inpath <- ("C:/Users/darth/Workdir")
setwd(Inpath) 

load("C:/Users/darth/Workdir/OTU_folder/GC_OtuOutput2_tabulated.Rdat")
...
setwd("C:/Users/darth/Workdir/OTU_folder")
...
#rep_set_OTUs <- as.data.frame(colnames(resM))
#rep_set_OTUs <- mutate(rep_set_OTUs, OTU_ID = 1:n())
#rep_set_OTUs$OTU_ID <- sub("^", "OTU_", rep_set_OTUs$OTU_ID) #renames the OTUs to be the OTU_ID, the sub function uses regex to find and replace the name using the ^ wildcard.
#rep_set_OTUs$OTU <- rep_set_OTUs$`colnames(resM)` #Adds the site/sample names to the OTU rep set
#rep_set_OTUs$`colnames(resM)` <- NULL
#colnames(resM) <- rep_set_OTUs$OTU_ID




#otus <- as.data.frame(otus)
#otus$OTU <- as.factor(rownames(otus))
#otus <- merge(rep_set_OTUs, otus, by = "OTU")
...
missing_otus <- setdiff(colnames(resM), rownames(otus)) #these otus couldn't be identified to any taxon by blast so end up in the resM but not the otu table

resM <- resM[, !colnames(resM) %in% missing_otus] #removes the missing otus from the resM as we have too little info about them
ncol(resM)

otus[is.na(otus)] <- "Unknown" #Next line of code will remove all NAs so NAs are now known as Unknowns.
otus_filt <- subset(otus, Domain != "Bacteria" & Domain != "Archaea" & Subdivision != "Fungi" & Subdivision != "Metazoa" 
                    & Class != "Embryophyceae" & Domain != "Eukaryota:mito" & Domain != "Eukaryota:plas" & Domain != "Unknown"
                    & Domain != "Eukaryota:chrom" & Domain != "Eukaryota:apic" & Domain != "Eukaryota:nucl")  #Filters out non-protists and mitochondrial/plastid sequences


to_remove <- anti_join(otus, otus_filt) #Finds out what is different between filtered and not filtered otus dfs.
to_remove <- rownames(to_remove) #Pulls out only the rownames (OTU_IDs)

resM_filt <- resM[ , !(colnames(resM) %in% to_remove)] #removes all the OTUs that did not pass the filter
...
#y axis is percent id
#x axis is sequence length
pdf(file = "OTU_match_plot.pdf", height=6,width=7)

par(mar=c(5,5,5,5))
par(mfrow =c(1,1))
plot(otus_filt$length, otus_filt$identity,  
     pch = otus_filt$pch[match(colnames(resM_nc),otus_filt$otu)],
     cex=sqrt(colSums(resM_nc)[match(otus_filt$otu, colnames(resM_nc))])/50,
     bg=otus_filt$colour[match(colnames(resM_nc), otus_filt$otu)],
     xlab = "Length of match (basepairs)", ylab = "Percent identity match", main = "OTU to database matching")

legend(1,75, legend = key_tax, pt.bg = tax_col, pch = tax_pch, cex = 0.7)
dev.off()



abline(h = 77.5, col = "red")
abline(v = 400, col = "red")


too_short <- otus_filt[otus_filt$length < 400,] #Finds otus that have too short of a match to the database
low_match <- otus_filt[otus_filt$identity < 77.5,] #Finds otus with low percent ID matches


otus_fin <- otus_filt[!rownames(otus_filt) %in% low_match$otu,]
otus_fin <- otus_filt[!rownames(otus_filt) %in% too_short$otu,]



to_remove2 <- anti_join(otus_filt, otus_fin)
to_remove2 <- rownames(to_remove2)

resM_Cons <- resM_filt_bl[ , !(colnames(resM_filt_bl) %in% to_remove2)]
...
pdf(file = "OTU_match_plot_post.pdf", height=6,width=7)

plot(otus_fin$length, otus_fin$identity,  
     pch = otus_fin$pch[match(colnames(resM_nc),otus_fin$otu)],
     cex=sqrt(colSums(resM_nc)[match(otus_filt$otu, colnames(resM_nc))])/50,
     bg=otus_fin$colour[match(colnames(resM_nc), otus_fin$otu)],
     xlab = "Length of match (basepairs)", ylab = "Percent identity match", main = "OTU to database matching")

legend(570, 75, legend = key_tax, pt.bg = tax_col, pch = tax_pch, cex = 0.7)
...
rared <- rarefy(resM_fin, sample = 4164)
rared


#rarecurve(resM_fin, sample = 4164)
resM_rare <- rrarefy(resM_fin, sample = 4164) #Used for richness
...
{
  rareStack[,,stack.i] <- rrarefy(resM_fin, 4164)
...
colnames(controls) <- otus_filt[colnames(controls),]$Species
...
prop_test <- sweep(sds, 1, rowSums(sds), "/")*100
site_veg <- rownames(prop_test)
site_veg <- gsub("GC...", "", site_veg)

cerc_prop <- data.frame(prop_test[,2], site_veg)
colnames(cerc_prop) <- c("Cercozoa", "Vegetation")
mean(cerc_prop$Cercozoa[pinesites])

cerctest <- aov(Cercozoa ~ Vegetation, data = cerc_prop)
plot(cerctest)
summary(cerctest)
TukeyHSD(cerctest)

#Cercozoa differ between pine and beech significantly. 

api_prop <- data.frame(prop_test[,6], site_veg)
colnames(api_prop) <- c("Apicomplexa", "Vegetation")
mean(api_prop$Apicomplexa)

apitest <- aov(Apicomplexa ~ Vegetation, data = api_prop)
plot(apitest)
summary(apitest)
TukeyHSD(apitest)

#Apis significantly different between pine and beech.


cili_prop <- data.frame(prop_test[,1], site_veg)
colnames(cili_prop) <- c("Ciliophora", "Vegetation")
mean(cili_prop$Ciliophora)

cilitest <- aov(Ciliophora ~ Vegetation, data = cili_prop)
plot(cilitest)
summary(cilitest)
TukeyHSD(cilitest)

#Beech has mroe ciliates than both pine and shrub


sups <- otus_fin[colnames(resM_fin),]$Supergroup

sps <- resM_fin
colnames(sps) <- sups
sps <- sps %*% sapply(unique(sups), "==", sups)

prop_test2 <- sweep(sps, 1, rowSums(sps), "/")*100

amo_prop <- data.frame(prop_test2[,3], site_veg)
colnames(amo_prop) <- c("Amoebozoa", "Vegetation")
mean(amo_prop$Amoebozoa)

amotest <- aov(Amoebozoa ~ Vegetation, data = amo_prop)
plot(amotest)
summary(amotest)
TukeyHSD(amotest)

#p.adjust(c(0.004, 0.02, 0.026,0.128), method = "holm") 

#No difference in Amoebozoa

other_group <- prop_test[,-c(1,2,6, 7, 10, 13, 14, 22 )]

othprop <- data.frame(rowSums(other_group), site_veg)
colnames(othprop) <- c("Other", "Vegetation")
mean(othprop$Other)

othtest <- aov(Other ~ Vegetation, data = othprop)
plot(othtest)
summary(othtest)
TukeyHSD(othtest)

#Significant difference between N and S 
...
shannon_barplot <- barplot(shannon_avg, main = "Effective diversity of each vegetation type", names = Veges, ylab = "Effective Diversity of OTUs", ylim = c(0,300), col = "white", cex.names = 1.5)
...
beta_dist2 <- raupcrick(resM_fin[1:32,], nsimul = 100)
...
#plot(beta_mds$points, vegtype_MDS, col = vegcol, pch = vegpch, main = "Community Dissimilarity of the Three Vegetation Types") #Colour points by vegtype. 
ordihull(beta_mds, vegtype_MDS, col = vegcol, lwd = 3)
#ordiellipse(beta_mds, vegtype_MDS, col = vegcol)
points(beta_mds$points, bg = vegcol[as.factor(vegtype_MDS)], pch = vegpch[as.factor(vegtype_MDS)], cex = 1.1)
legend(0.05,-0.19, legend = Veges, col = vegcol, pch = vegpch, cex = 1.4, pt.bg = vegcol)
...
#pdf(file = 'compositionplot.pdf', width =8.3, height = 11.7)
...
par(mar=c(0,1,0,0))
par(mai=c(1,0.5,0.6,0))
par(mgp = c(2, 0.4,0), las=1)
...
legend(0.2, 300, legend = key_tax, pt.bg = tax_col, pch = 21, cex = 1.4)
...
otus_fin$colour <- othercol


otus_fin$colour[otus_fin$Subdivision == "Cercozoa"] <- cercocol
otus_fin$colour[otus_fin$Subdivision == "Apicomplexa"] <- apicol
otus_fin$colour[otus_fin$Subdivision == "Ciliophora"] <- cilicol
#otus_fin$colour[otus_fin$Subdivision == "Unknown"] <- unkcol
otus_fin$colour[otus_fin$Supergroup == "Amoebozoa"] <- Amoebocol

otus_fin$pch <- otherpch

otus_fin$pch[otus_fin$Subdivision == "Cercozoa"] <- cercopch
otus_fin$pch[otus_fin$Subdivision == "Apicomplexa"] <- apipch
otus_fin$pch[otus_fin$Subdivision == "Ciliophora"] <- cilipch
#otus_fin$pch[otus_fin$Subdivision == "Unknown"] <- unkpch
otus_fin$pch[otus_fin$Supergroup == "Amoebozoa"] <- amoebpch
...
#pdf(file = 'rarityplot.pdf', width =8.3, height = 11.7)
...
notho_freq_vs_dom <- plot(notho_dom ~ jitter(notho_freq), log = "xy", bg = otus_fin$colour[match(names(notho_dom), otus_fin$otu)],
                          pch = otus_fin$pch[match(names(notho_dom), otus_fin$otu)], main = "Beech", ,  xlab = "", ylab = "Dominance (%)", cex = 2, cex.main = 2, ylim = c(0.0002, 4))

abline(h = 1, col = "red", lwd =  1.5)
#axis(side = 2, at = c(range(round(notho_dom, digits = 4)), 0.001, 0.005, 0.01, 0.05, 0.1, 0.5, 1))
#axis(side = 1, at = c(1, 2, 5, 11))


shrub_freq_vs_dom <- plot(shrub_dom ~ jitter(shrub_freq), log = "xy", bg = otus_fin$colour[match(names(shrub_dom),otus_fin$otu)], 
                          pch = otus_fin$pch[match(names(shrub_dom), otus_fin$otu)], main = "Shrubland",  xlab = "Frequency", ylab = "", cex = 2, cex.main = 2, ylim = c(0.0002, 4))

abline(h = 1, col = "red", lwd =  1.5)
#axis(side = 2, at = c(range(round(shrub_dom, digits = 4)), 0.001, 0.005, 0.01, 0.05, 0.1, 0.5, 1))
#axis(side = 1, at = c(1, 2, 5, 11))

pine_freq_vs_dom <- plot(pine_dom ~ jitter(pine_freq), log = "xy", bg = otus_fin$colour[match(names(pine_dom),otus_fin$otu)], 
                         pch = otus_fin$pch[match(names(pine_dom),otus_fin$otu)], main = "Pine",  xlab = "", ylab = "", cex =2, cex.main = 2, ylim = c(0.0002, 4))

abline(h = 1, col = "red", lwd =  1.5)
#axis(side = 2, at = c(range(round(pine_dom, digits = 4)), 0.001, 0.005, 0.01, 0.05, 0.1, 0.5, 1))
#axis(side = 1, at = c(1, 2, 5, 11))




legend(2, 0.0005, legend = key_tax, pt.bg = tax_col, pch = tax_pch, cex = 1.2)
